...
const TaskCard = ({item, onClick}) => {
    const {title, assignedTo, completedTodoCount, createdAt, description, dueDate, priority, progress, status, todoChecklist, attachments} = item;
  ...
  return (
    <div className='bg-white rounded-xl py-4 hover:shadow-md shadow-gray-100 border border-gray-200/50 cursor-pointer' onClick={onClick}>
      ...
      >
        <p className='text-sm font-medium text-gray-500 mt-4 line-clamp-2'>{title}</p>
      ...
      <div className='px-4'>
        ...
        <div className='flex items-center justify-between mt-3'>
            <AvatarGroup avatars={assignedTo?.map((assign) => assign.profileImageUrl) || null} />
